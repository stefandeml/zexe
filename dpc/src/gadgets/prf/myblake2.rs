...
//     Engine,
// };
// also got re-named from Engine to PairingEngine in zEXE
...
use snark_gadgets::{boolean::Boolean, uint32::UInt32};
...
mod test {
    ...
    //     // >>> h.hexdigest()
    //     let expected =
    // hex!("c59f682376d137f3f255e671e207d1f2374ebe504e9314208a52d9f88d69e8c8");
    ...
    fn test_blake2s_constraints() {
        ...
        let input_bits: Vec<_> = (0..512)
            .map(|i| {
                AllocatedBit::alloc(cs.ns(|| format!("input bit {}", i)), || Ok(true))
                    .unwrap()
                    .into()
            })
            .collect();
        // let input_bits: Vec<_> = (0..512).map(|i| AllocatedBit::alloc(cs.namespace(||
        // format!("input bit {}", i)), Some(true)).unwrap().into()).collect();
    ...
    // 0xe5bc0654]);     let input_bits: Vec<_> = (0..512)
    //       .map(|_| Boolean::constant(rng.gen()))
    //       .chain((0..512)
    //                     .map(|i| AllocatedBit::alloc(cs.namespace(||
    // format!("input bit {}", i)), Some(true)).unwrap().into()))
    //       .collect();
    //     blake2s(&mut cs, &input_bits, b"12345678").unwrap();
    //     assert!(cs.is_satisfied());
    //     assert_eq!(cs.num_constraints(), 21518);
    ...
    //     let mut cs = TestConstraintSystem::<Bls12>::new();
    //     let mut rng = XorShiftRng::from_seed([0x5dbe6259, 0x8d313d76, 0x3237db17,
    // 0xe5bc0654]);     let input_bits: Vec<_> = (0..512).map(|_|
    // Boolean::constant(rng.gen())).collect();     blake2s(&mut cs,
    // &input_bits, b"12345678").unwrap();     assert_eq!(cs.num_constraints(),
    // 0); }
    ...
    //             for bit_i in 0..8 {
    //                 let cs = cs.namespace(|| format!("input bit {} {}", byte_i,
    // bit_i));

    //                 input_bits.push(AllocatedBit::alloc(cs, Some((input_byte >>
    // bit_i) & 1u8 == 1u8)).unwrap().into());             }
    //         }
