...
//     Field
// };
use super::{wnaf::Wnaf, Parameters, VerifyingKey};
use algebra::{
...
// };
// use domain by zExe
use algebra::fft::domain::{EvaluationDomain, Scalar};
    ...
    C: Circuit<E>,
{
    ...
    let mut powers_of_tau = EvaluationDomain::<E, _>::from_coeffs(powers_of_tau).unwrap();
    ...
    let mut h = vec![E::G1Projective::zero(); powers_of_tau.as_ref().len() - 1];
    {
        ...
        worker.scope(h.len(), |scope, chunk| {
                ...
                .zip(powers_of_tau.as_ref().chunks(chunk))
            {
                ...
                scope.spawn(move |_| {
                    ...
                    // Batch normalize
                    E::G1Projective::batch_normalization(h);
                });
            }
        });
    ...
    // elog_verbose!("using inverse FFT to convert powers of tau to Lagrange
    // coefficients...");
    ...
    let mut a = vec![E::G1Projective::zero(); assembly.num_inputs + assembly.num_aux];
    let mut b_g1 = vec![E::G1Projective::zero(); assembly.num_inputs + assembly.num_aux];
    let mut b_g2 = vec![E::G2Projective::zero(); assembly.num_inputs + assembly.num_aux];
    let mut ic = vec![E::G1Projective::zero(); assembly.num_inputs];
    let mut l = vec![E::G1Projective::zero(); assembly.num_aux];
    ...
    fn eval<E: Engine>(
        // wNAF window tables
        g1_wnaf: &Wnaf<usize, &[E::G1Projective], &mut Vec<i64>>,
        g2_wnaf: &Wnaf<usize, &[E::G2Projective], &mut Vec<i64>>,
        ...
        b_g1: &mut [E::G1Projective],
        b_g2: &mut [E::G2Projective],
        ext: &mut [E::G1Projective],
        ...
        worker.scope(a.len(), |scope, chunk| {
                ...
                .zip(ct.chunks(chunk))
            {
                ...
                scope.spawn(move |_| {
                    ...
                    // Batch normalize
                    E::G1Projective::batch_normalization(a);
                    E::G1Projective::batch_normalization(b_g1);
                    E::G2Projective::batch_normalization(b_g2);
                    E::G1Projective::batch_normalization(ext);
                });
            }
        });
    }
